...
var h1 = token(P.seq(sharp, whitespace).then(plainStr).or(plainStr.skip(P.seq(linebreak, equal.atLeast(1), linebreak)))).map(surroundWith("h1"));
var h2 = token(P.seq(sharp.times(2), whitespace).then(plainStr).or(plainStr.skip(P.seq(linebreak, minus.atLeast(1), linebreak)))).map(surroundWith("h2"));
...
var paragraphOrLinebreak = P.seq(paragraph, paragraphBreak, paragraph).map(function (x) { return x.join(""); })
    .or(paragraph)
    .atLeast(1)
...
var acceptables = P.alt(h6, h5, h4, h3, h2, h1, table, lists, codeBlock, blockquote, paragraphOrLinebreak, linebreak.result("")).many().map(function (x) { return x.join(""); });

...
var inline = P.alt(anchor, em, strong, paragraphStr);
...
var lists = ul.or(ol);
var codeBegin = P.string("```");
var codeEnd = P.string("```");
var codeStr = P.regexp(/[^`]*/);
var code = codeBegin.then(codeStr).skip(codeEnd).map(surroundWith("code"));
var acceptables = P.alt(h6, h5, h4, h3, h2, h1, lists, code, paragraphOrLinebreak, whitespace.result("<br />")).many().map(function (x) { return x.join(""); });

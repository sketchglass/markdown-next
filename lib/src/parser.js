...
class Parser {
    ...
    create() {
        ...
        const inline = P.alt(pluginInline, anchor, img, em, strong, code, P.regexp(/[^\r\n=-\[\]\*\`\@]+/), P.regexp(/./));
        ...
        const table = P.seqMap(tableHeader, tableHSep, tableBody.atLeast(1), (headers, _1, bodies) => mapper("table")(join([
        ...
        let liLevel = null;
        let liLevelBefore = null;
        let nodeType;
        const listLineContent = P.seqMap(P.seqMap(listIndent.many(), P.index, (_1, index) => {
            const _index = index;
            if (liLevelBefore === null)
                liLevelBefore = liLevel = _index.column;
            liLevelBefore = liLevel;
            liLevel = _index.column;
        }), ulStart.or(olStart), (_1, start) => {
        ...
        const codeBlock = P.seqMap(codeBlockBegin, codeBlockDefinitionStr, linebreak, linebreak.or(codeBlockStr.skip(linebreak)).many(), codeBlockEnd, (_1, definition, _2, code, _3) => {
        ...
        const blockquoteLine = P.lazy(() => {
            ...
            return P.seqMap(blockquoteBegin.then(blockquoteBegin.many().map(x => blockquoteLevel = x.length)), blockquoteStr, linebreak.atMost(1), (_1, text, _2) => {
        ...
        const block = P.alt(P.regexp(/\s+/).result(""), pluginBlock, lists, h1Special, h2Special, h6, h5, h4, h3, h2, h1, table, codeBlock, blockquote, paragraph, linebreak.result(""));
